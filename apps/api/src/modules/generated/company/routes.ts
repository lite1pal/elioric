...
export interface CompanyRoutesOptions {
  ...
  service: ReturnType<typeof createCompanyService>;
}
export async function registerCompanyRoutes(
  app: FastifyInstance,
  options: CompanyRoutesOptions
) {
  app.get("/v1/organizations/:organizationId/companies", async (request, reply) => {
  ...
  app.post("/v1/organizations/:organizationId/companies", async (request, reply) => {
    const user = request.sessionUser;
    const params = organizationParamsSchema.safeParse(request.params);
    if (!user) {
      return reply.code(401).send({ error: "missing_session" });
  ...
  app.get("/v1/organizations/:organizationId/companies/:id", async (request, reply) => {
    const user = request.sessionUser;
  ...
  app.patch("/v1/organizations/:organizationId/companies/:id", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
    if (!user) {
      return reply.code(401).send({ error: "missing_session" });
  ...
  app.post("/v1/organizations/:organizationId/companies/:id/archive", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
  ...
  app.post("/v1/organizations/:organizationId/companies/:id/unarchive", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
