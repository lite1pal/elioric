...
const generatedResourcePolicy = {
  archive: { mode: "ownership-aware", ownerField: "ownerId" },
  read: { mode: "ownership-aware", ownerField: "ownerId" },
  workflow: { mode: "ownership-aware", ownerField: "ownerId" },
  write: { mode: "ownership-aware", ownerField: "ownerId" }
} as const satisfies Record<GeneratedResourcePolicyAction, GeneratedResourcePolicyRule>;
export interface DealRoutesOptions {
  access: {
  ...
  service: ReturnType<typeof createDealService>;
}
export async function registerDealRoutes(
  app: FastifyInstance,
  options: DealRoutesOptions
) {
  app.get("/v1/organizations/:organizationId/deals", async (request, reply) => {
  ...
  app.post("/v1/organizations/:organizationId/deals", async (request, reply) => {
    const user = request.sessionUser;
    const params = organizationParamsSchema.safeParse(request.params);
    if (!user) {
      return reply.code(401).send({ error: "missing_session" });
  ...
  app.get("/v1/organizations/:organizationId/deals/:id", async (request, reply) => {
    const user = request.sessionUser;
  ...
  app.patch("/v1/organizations/:organizationId/deals/:id", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
    if (!user) {
      return reply.code(401).send({ error: "missing_session" });
  ...
  app.post("/v1/organizations/:organizationId/deals/:id/archive", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
  ...
  app.post("/v1/organizations/:organizationId/deals/:id/unarchive", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
