...
export async function registerNoteRoutes(
  ...
  app.get("/v1/organizations/:organizationId/notes", async (request, reply) => {
  ...
  app.post("/v1/organizations/:organizationId/notes", async (request, reply) => {
    const user = request.sessionUser;
    const params = organizationParamsSchema.safeParse(request.params);
    if (!user) {
      return reply.code(401).send({ error: "missing_session" });
  ...
  app.get("/v1/organizations/:organizationId/notes/:id", async (request, reply) => {
    const user = request.sessionUser;
  ...
  app.patch("/v1/organizations/:organizationId/notes/:id", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
    if (!user) {
      return reply.code(401).send({ error: "missing_session" });
  ...
  app.post("/v1/organizations/:organizationId/notes/:id/archive", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
  ...
  app.post("/v1/organizations/:organizationId/notes/:id/unarchive", async (request, reply) => {
    const user = request.sessionUser;
    const params = resourceIdParamsSchema.safeParse(request.params);
