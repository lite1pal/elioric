import type { ApiClient } from "@/src/lib/api/api-client";
import { dealRecordSchema } from "@/src/features/deal/domain/schemas";
import { z } from "zod";
const dealPageInfoSchema = z.object({
  hasMore: z.boolean(),
  nextCursor: z.string().nullable()
});
const dealListResponseSchema = z.object({
  items: z.array(dealRecordSchema),
  pageInfo: dealPageInfoSchema
});
export function createResourceClient(apiClient: ApiClient) {
  return {
    async create(organizationId: string, body: Record<string, unknown>) {
      return dealRecordSchema.parse(
        await apiClient.request({
          body,
          method: "POST",
          path: `/api/v1/organizations/${organizationId}/deals` as never
        })
      );
    },
    async get(organizationId: string, id: string) {
      return dealRecordSchema.parse(
        await apiClient.request({
          path: `/api/v1/organizations/${organizationId}/deals/${id}` as never
        })
      );
    },
    async list(
      organizationId: string,
      options?: {
archived?: "exclude" | "include" | "only";
cursor?: string;
limit?: number;
query?: string;
sortBy?: "createdAt" | "updatedAt" | "name";
sortDirection?: "asc" | "desc";
    stage?: "lead" | "qualified" | "proposal" | "won" | "lost";
    companyId?: string;
    ownerId?: string;
      }
    ) {
      return dealListResponseSchema.parse(
        await apiClient.request({
          path: `/api/v1/organizations/${organizationId}/deals${buildListQuery(options)}` as never
        })
      );
    },
    async update(organizationId: string, id: string, body: Record<string, unknown>) {
      return dealRecordSchema.parse(
        await apiClient.request({
          body,
          method: "PATCH",
          path: `/api/v1/organizations/${organizationId}/deals/${id}` as never
        })
      );
    },
    async archive(organizationId: string, id: string) {
      return dealRecordSchema.parse(
        await apiClient.request({
          method: "POST",
          path: `/api/v1/organizations/${organizationId}/deals/${id}/archive` as never
        })
      );
    },
    async unarchive(organizationId: string, id: string) {
      return dealRecordSchema.parse(
        await apiClient.request({
          method: "POST",
          path: `/api/v1/organizations/${organizationId}/deals/${id}/unarchive` as never
        })
      );
    }
  };
}
function buildListQuery(options?: {
archived?: "exclude" | "include" | "only";
cursor?: string;
limit?: number;
query?: string;
sortBy?: "createdAt" | "updatedAt" | "name";
sortDirection?: "asc" | "desc";
    stage?: "lead" | "qualified" | "proposal" | "won" | "lost";
    companyId?: string;
    ownerId?: string;
}) {
  const query = new URLSearchParams();
  for (const [key, value] of Object.entries({
    archived: options?.archived,
    cursor: options?.cursor,
    limit: options?.limit,
    query: options?.query,
    sortBy: options?.sortBy,
    sortDirection: options?.sortDirection,
    stage: options?.stage,
    companyId: options?.companyId,
    ownerId: options?.ownerId,
  })) {
    if (value === undefined || value === null || value === "") {
      continue;
    }
    query.set(key, String(value));
  }
  const queryString = query.toString();
  return queryString.length > 0 ? `?${queryString}` : "";
}
