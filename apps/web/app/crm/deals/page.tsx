...
export default async function ResourcePage({ searchParams }: ResourcePageProps) {
  ...
  const data = await loadDealWorkspacePage(resolvedSearchParams, {
    currentUser
  });
    ...
    >
      <div className="grid gap-6">
        <header className="grid gap-2">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--muted)]">Deals</p>
        ...
        {data.workspace.activeOrganizationId ? (
          ...
          <label className="grid gap-2">
            <span>Stage</span>
            <select className="rounded-md border border-[var(--border)] px-3 py-2" defaultValue={data.listQuery.stage ?? ""} name="stage">
